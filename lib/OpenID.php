...
class OpenID {
  ...
  private static function getStore() {
    $storePath = '/tmp/civvic_openidStorePath';
  ...
  private static function getReturnTo() {
    return Util::getFullServerUrl() . "/auth/revenireOpenid";
  }
  ...
  static function beginAuth($openid, $policyUris) {
    ...
    if ($authRequest->shouldSendRedirect()) {
      ...
      if (Auth_OpenID::isFailure($formHtml)) {
        FlashMessage::add('Nu vă putem redirecționa către serverul OpenID: ' . $formHtml->message);
        return false;
      } else {
        print $formHtml;
      }
  ...
  static function finishAuth() {
    ...
    if ($response->status == Auth_OpenID_CANCEL) {
      FlashMessage::add('Verificare anulată.', 'warning');
      return null;
